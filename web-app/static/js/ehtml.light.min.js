...
// src/responseFromAjaxRequest.js
var responseFromAjaxRequest_default = (options, requestBody, callback) => {
  let resObj = {};
  ...
  if (options.overrideMimeType !== void 0) {
  ...
  req.onreadystatechange = function() {
    if (req.readyState === req.DONE) {
      ...
      let headerMap = {};
      let headers2 = allHeadersStr.split(/[\r\n]+/);
      headers2.forEach((line) => {
...
// src/evaluatedStringWithParamsFromState.js
var pattern = /\${([^}]+)}/g;
function evaluatedStringWithParamsFromState_default(string, state, node) {
  if (!string) {
    return null;
  }
  state = state || {};
...
// src/evaluateStringWithActionsOnProgress.js
function evaluateStringWithActionsOnProgress_default(string, node) {
...
// src/unwrappedChildrenOfParent.js
function unwrappedChildrenOfParent_default(parent) {
...
// src/evaluateStringWithActionsOnResponse.js
function evaluateStringWithActionsOnResponse_default(string, resName, resObj, node) {
...
var e_json_default = (node) => {
  ...
  if (cacheFromAttribute) {
    const evaluatedCacheAsString = evaluatedStringWithParamsFromState_default(
      cacheFromAttribute,
      node.__ehtmlState__,
      node
    );
...
// src/elm.js
var elm_default = (elmSelectorOrElm) => {
...
// src/isTemplate.js
function isTemplate_default(node) {
  return node.nodeName.toLowerCase() === "template";
}

// src/isTemplateWithType.js
function isTemplateWithType_default(node, type) {
  if (isTemplate_default(node)) {
...
// src/evaluatedStringWithParams.js
var pattern2 = /\${([^}]+)}/g;
function evaluatedStringWithParams_default(string, node) {
  if (!string) {
    return null;
  }
  return string.replace(pattern2, (match, expression) => {
    const inlinedExpression = expression.replace(/\n/g, " ");
...
// src/observeNodeAttributes.js
var ATTRIBUTE_NAMES_TO_IGNORE_SINCE_THEY_MUST_BE_RESOLVED_IN_THEIR_OWN_SCOPE_AND_TIME = [
...
function observeNodeAttributes_default(node, state) {
  if (node.attributes) {
    const nodeAttributes = Array.from(node.attributes);
    for (let i = 0; i < nodeAttributes.length; i++) {
      const attr = nodeAttributes[i];
      const isAttributeToBeIgnored = ATTRIBUTE_NAMES_TO_IGNORE_SINCE_THEY_MUST_BE_RESOLVED_IN_THEIR_OWN_SCOPE_AND_TIME.indexOf(attr.name) >= 0 || attr.name === "data-src" && TAGS_WITH_SRC_ATTRIBUTE.indexOf(node.tagName.toLowerCase()) === -1;
      ...
      node.setAttribute(
        attr.name,
        state ? evaluatedStringWithParamsFromState_default(attr.value, state, node) : evaluatedStringWithParams_default(attr.value, node)
      );
...
// src/releaseTemplateWithItsContent.js
function releaseTemplateWithItsContent_default(template, contentNode) {
  if (isTemplateWithType_default(template, "e-reusable")) {
...
function mapToTemplate(elmSelectorOrElm, obj) {
  const mappingElement = elm_default(elmSelectorOrElm);
  if (mappingElement === null || mappingElement === void 0) {
    throw new Error("Mapping element is not found");
  }
  if (!isTemplate_default(mappingElement)) {
    throw new Error("Mapping element must be <template>");
  }
  if (isTemplateWithType_default(mappingElement, "e-if")) {
  ...
  if (isTemplateWithType_default(mappingElement, "e-for-each")) {
  ...
  releaseTemplateWithItsContent_default(mappingElement, elmContentNode);
}
function map(elmContentNode, state) {
  iterateChildNodes(
    ...
    (node) => {
      ...
      if (isTemplateWithType_default(node, "e-for-each")) {
        activateEForEach(node, overridenState);
        node.observedByEHTML = true;
        node.activatedByHTML = true;
      } else if (isTemplateWithType_default(node, "e-if")) {
        activateEIf(node, overridenState);
        node.observedByEHTML = true;
        node.activatedByHTML = true;
      } else {
        observeNodeAttributes_default(node, overridenState);
...
function activateEIf(node, state) {
  ...
  const toDisplay = evaluatedStringWithParamsFromState_default(
    inlinedToDisplayExpression,
    state,
    node
  ).trim();
  if (toDisplay === "true") {
...
function activateEForEach(node, state) {
  ...
  const listDefinitionExpressionBody = evaluatedStringWithParamsFromState_default(
  ...
  list.forEach((item, index) => {
    if (typeof item === "object" && item["index"] === void 0) {
      item.index = index + 1;
    }
...
// src/E/e-json-template.js
var e_json_template_default = (node) => {
  const ajaxIconSelector = node.getAttribute("data-ajax-icon");
  const ajaxIcon = document.querySelector(ajaxIconSelector);
  if (ajaxIcon) {
    ajaxIcon.style.display = "";
...
// src/third-party/json-pretty-html.js
var matchHtmlRegExp = /["'&<>]/;
...
function json_pretty_html_default(object, selection, options) {
...
// src/E/e-json-view.js
var e_json_view_default = (node) => {
  if (node.hasAttribute("data-actions-on-progress-start")) {
    evaluateStringWithActionsOnProgress_default(
      node.getAttribute("data-actions-on-progress-start"),
      node
  ...
  responseFromAjaxRequest_default({
    ...
    node.innerHTML = json_pretty_html_default(
...
var e_if_template_default = (node) => {
  const toDisplayExpression = node.getAttribute("data-condition-to-display");
  if (!toDisplayExpression) {
    throw new Error('e-if must have "data-condition-to-display" attribute');
  }
  const toDisplay = evaluatedStringWithParams_default(toDisplayExpression);
  if (toDisplay === "true") {
    const contentNode = node.content.cloneNode(true);
    node.parentNode.insertBefore(contentNode, node);
  }
...
var e_for_each_template_default = (node) => {
...
function validateFormElement(form, element, requestBody, queryObject) {
  ...
  let value = requestBody[nameAttribute] === void 0 ? queryObject[nameAttribute] : requestBody[nameAttribute];
  ...
  form.validationErrorBoxes.forEach((elementWithErrorMessageBox) => {
    if (elementWithErrorMessageBox.parentNode) {
      elementWithErrorMessageBox.parentNode.replaceChild(
        elementWithErrorMessageBox.firstChild,
...
var e_form_dynamic_value_default = (node) => {
  node.style.display = "none";
  node.name = node.getAttribute("name");
  node.value = () => {
    return evaluatedStringWithParamsFromState_default(
...
// src/E/e-local-storage-value.js
var e_local_storage_value_default = (node) => {
  node.name = node.getAttribute("name");
  node.value = () => {
...
// src/E/e-session-storage-value.js
var e_session_storage_value_default = (node) => {
  node.name = node.getAttribute("name");
  node.value = () => {
...
// src/E/e-github-oauth-button.js
var e_github_oauth_button_default = (node) => {
...
// src/E/e-page-with-url-template.js
var e_page_with_url_template_default = (node) => {
...
// src/E/e-reusable-template.js
var e_reusable_template_default = (node) => {
};

// src/E/e-select.js
var e_select_default = (node) => {
...
var ce = (b, y, r, A) => {
  if (y && typeof y == "object" || typeof y == "function") for (let z of ue(y)) !ae.call(b, z) && z !== r && O(b, z, { get: () => y[z], enumerable: !(A = de(y, z)) || A.enumerable });
  return b;
};
var se = (b, y, r) => (r = b != null ? ee(re(b)) : {}, ce(y || !b || !b.__esModule ? O(r, "default", { value: b, enumerable: true }) : r, b));
var K = ne((W, B) => {
  (function() {
    function b(e) {
      "use strict";
      ...
      if (e === false) return JSON.parse(JSON.stringify(u));
      var d = {};
      for (var a in u) u.hasOwnProperty(a) && (d[a] = u[a].defaultValue);
      return d;
    }
    function y() {
      "use strict";
      var e = b(true), u = {};
      for (var d in e) e.hasOwnProperty(d) && (u[d] = true);
      return u;
    }
    var r = {}, A = {}, z = {}, E = b(true), $ = "vanilla", L = { github: { omitExtraWLInCodeBlocks: true, simplifiedAutoLink: true, excludeTrailingPunctuationFromURLs: true, literalMidWordUnderscores: true, strikethrough: true, tables: true, tablesHeaderId: true, ghCodeBlocks: true, tasklists: true, disableForced4SpacesIndentedSublists: true, simpleLineBreaks: true, requireSpaceBeforeHeadingText: true, ghCompatibleHeaderId: true, ghMentions: true, backslashEscapesHTMLTags: true, emoji: true, splitAdjacentBlockquotes: true }, original: { noHeaderId: true, ghCodeBlocks: false }, ghost: { omitExtraWLInCodeBlocks: true, parseImgDimensions: true, simplifiedAutoLink: true, excludeTrailingPunctuationFromURLs: true, literalMidWordUnderscores: true, strikethrough: true, tables: true, tablesHeaderId: true, ghCodeBlocks: true, tasklists: true, smoothLivePreview: true, simpleLineBreaks: true, requireSpaceBeforeHeadingText: true, ghMentions: false, encodeEmails: true }, vanilla: b(true), allOn: y() };
    r.helper = {}, r.extensions = {}, r.setOption = function(e, u) {
      ...
      "use strict";
      if (r.helper.isString(e)) if (typeof u < "u") A[e] = u;
      else {
      ...
      if (e = r.helper.stdExtName(e), r.helper.isUndefined(u)) {
        if (!z.hasOwnProperty(e)) throw Error("Extension named " + e + " is not registered!");
        return z[e];
      } else {
    ...
    function H(e, u) {
      ...
      for (var s = 0; s < e.length; ++s) {
        ...
        if (typeof c != "object") return a.valid = false, a.error = i + "must be an object, but " + typeof c + " given", a;
        if (!r.helper.isString(c.type)) return a.valid = false, a.error = i + 'property "type" must be a string, but ' + typeof c.type + " given", a;
        var t = c.type = c.type.toLowerCase();
        if (t === "language" && (t = c.type = "lang"), t === "html" && (t = c.type = "output"), t !== "lang" && t !== "output" && t !== "listener") return a.valid = false, a.error = i + "type " + t + ' is not recognized. Valid values: "lang/language", "output/html" or "listener"', a;
        if (t === "listener") {
          if (r.helper.isUndefined(c.listeners)) return a.valid = false, a.error = i + '. Extensions of type "listener" must have a property called "listeners"', a;
        } else if (r.helper.isUndefined(c.filter) && r.helper.isUndefined(c.regex)) return a.valid = false, a.error = i + t + ' extensions must define either a "regex" property or a "filter" method', a;
        if (c.listeners) {
          if (typeof c.listeners != "object") return a.valid = false, a.error = i + '"listeners" property must be an object but ' + typeof c.listeners + " given", a;
          for (var p in c.listeners) if (c.listeners.hasOwnProperty(p) && typeof c.listeners[p] != "function") return a.valid = false, a.error = i + '"listeners" property must be an hash of [event name]: [callback]. listeners.' + p + " must be a function but " + typeof c.listeners[p] + " given", a;
        }
        if (c.filter) {
          if (typeof c.filter != "function") return a.valid = false, a.error = i + '"filter" must be a function, but ' + typeof c.filter + " given", a;
        } else if (c.regex) {
          if (r.helper.isString(c.regex) && (c.regex = new RegExp(c.regex, "g")), !(c.regex instanceof RegExp)) return a.valid = false, a.error = i + '"regex" property must either be a string or a RegExp object, but ' + typeof c.regex + " given", a;
          if (r.helper.isUndefined(c.replace)) return a.valid = false, a.error = i + '"regex" extensions must implement a replace string or function', a;
        }
      }
      return a;
    }
    r.validateExtension = function(e) {
      ...
      if (r.helper.isUndefined(u)) throw new Error("callback param is required");
      if (!r.helper.isFunction(u)) throw new Error("callback param must be a function/closure");
      if (typeof e.forEach == "function") e.forEach(u);
      else if (r.helper.isArray(e)) for (var d = 0; d < e.length; d++) u(e[d], d, e);
      else if (typeof e == "object") for (var a in e) e.hasOwnProperty(a) && u(e[a], a, e);
    ...
    r.helper.escapeCharactersCallback = D, r.helper.escapeCharacters = function(e, u, d) {
      "use strict";
      var a = "([" + u.replace(/([\[\]\\])/g, "\\$1") + "])";
      d && (a = "\\\\" + a);
    ...
    r.helper.matchRecursiveRegExp = function(e, u, d, a) {
      "use strict";
      for (var s = F(e, u, d, a), i = [], c = 0; c < s.length; ++c) i.push([e.slice(s[c].wholeMatch.start, s[c].wholeMatch.end), e.slice(s[c].match.start, s[c].match.end), e.slice(s[c].left.start, s[c].left.end), e.slice(s[c].right.start, s[c].right.end)]);
      return i;
      ...
      var c = F(e, d, a, s), t = e, p = c.length;
      if (p > 0) {
        var l = [];
        c[0].wholeMatch.start !== 0 && l.push(e.slice(0, c[0].wholeMatch.start));
        for (var o = 0; o < p; ++o) l.push(u(e.slice(c[o].wholeMatch.start, c[o].wholeMatch.end), e.slice(c[o].match.start, c[o].match.end), e.slice(c[o].left.start, c[o].left.end), e.slice(c[o].right.start, c[o].right.end))), o < p - 1 && l.push(e.slice(c[o].wholeMatch.end, c[o + 1].wholeMatch.start));
      ...
      if (!(u instanceof RegExp)) throw "InvalidArgumentError: second parameter of showdown.helper.regexIndexOf function must be an instance of RegExp";
      ...
      return e = e.replace(/./g, function(d) {
        if (d === "@") d = u[Math.floor(Math.random() * 2)](d);
        else {
          var a = Math.random();
      ...
      "use strict";
      var u = {}, d = [], a = [], s = {}, i = $, c = { parsed: {}, raw: "", format: "" };
      t();
      function t() {
        e = e || {};
        for (var n in E) E.hasOwnProperty(n) && (u[n] = E[n]);
        if (typeof e == "object") for (var f in e) e.hasOwnProperty(f) && (u[f] = e[f]);
      ...
      function p(n, f) {
        if (f = f || null, r.helper.isString(n)) if (n = r.helper.stdExtName(n), f = n, r.extensions[n]) {
          ...
          return;
        } else if (!r.helper.isUndefined(z[n])) n = z[n];
        else throw Error('Extension "' + n + '" could not be loaded. It was either not found or is not a valid extension.');
        typeof n == "function" && (n = n()), r.helper.isArray(n) || (n = [n]);
        var _ = H(n, f);
        if (!_.valid) throw Error(_.error);
        for (var m = 0; m < n.length; ++m) {
          ...
          if (n[m].hasOwnProperty("listeners")) for (var w in n[m].listeners) n[m].listeners.hasOwnProperty(w) && o(w, n[m].listeners[w]);
        }
      }
      function l(n, f) {
        ...
        var _ = H(n, f);
        if (!_.valid) throw Error(_.error);
        for (var m = 0; m < n.length; ++m) switch (n[m].type) {
          case "lang":
            d.push(n[m]);
            break;
          case "output":
      ...
      function o(n, f) {
        if (!r.helper.isString(n)) throw Error("Invalid argument in converter.listen() method: name must be a string, but " + typeof n + " given");
      ...
      this._dispatch = function(f, _, m, w) {
        if (s.hasOwnProperty(f)) for (var g = 0; g < s[f].length; ++g) {
...
`, n = r.subParser("detab")(n, u, f), n = n.replace(/^[ \t]+$/mg, ""), r.helper.forEach(d, function(_) {
        ...
        if (n = n.replace(/\r\n/g, `
`), n = n.replace(/\r/g, `
`), n = n.replace(/>[ \t]+</, ">\xA8NBSP;<"), !f) if (window && window.document) f = window.document;
        ...
        var m = { preList: S(_) };
        j(_);
        for (var w = _.childNodes, g = "", v = 0; v < w.length; v++) g += r.subParser("makeMarkdown.node")(w[v], m);
        function j(k) {
          for (var P = 0; P < k.childNodes.length; ++P) {
            var C = k.childNodes[P];
            C.nodeType === 3 ? !/\S/.test(C.nodeValue) && !/^[ ]+$/.test(C.nodeValue) ? (k.removeChild(C), --P) : (C.nodeValue = C.nodeValue.split(`
`).join(" "), C.nodeValue = C.nodeValue.replace(/(\s)+/g, "$1")) : C.nodeType === 1 && j(C);
          }
        }
        function S(k) {
          for (var P = k.querySelectorAll("pre"), C = [], M = 0; M < P.length; ++M) if (P[M].childElementCount === 1 && P[M].firstChild.tagName.toLowerCase() === "code") {
            var I = P[M].firstChild.innerHTML.trim(), N = P[M].firstChild.getAttribute("data-language") || "";
            if (N === "") for (var q = P[M].firstChild.className.split(" "), V = 0; V < q.length; ++V) {
              var G = q[V].match(/^language-(.+)$/);
              if (G !== null) {
                N = G[1];
                break;
              }
            }
            I = r.helper.unescapeHTMLEntities(I), C.push(I), P[M].outerHTML = '<precode language="' + N + '" precodenum="' + M.toString() + '"></precode>';
          } else C.push(P[M].innerHTML), P[M].innerHTML = "", P[M].setAttribute("prenum", M.toString());
          return C;
        }
        return g;
        ...
        if (!L.hasOwnProperty(n)) throw Error(n + " flavor was not found");
        var f = L[n];
        i = n;
        for (var _ in f) f.hasOwnProperty(_) && (u[_] = f[_]);
      }, this.getFlavor = function() {
        return i;
      }, this.removeExtension = function(n) {
        r.helper.isArray(n) || (n = [n]);
        for (var f = 0; f < n.length; ++f) {
          for (var _ = n[f], m = 0; m < d.length; ++m) d[m] === _ && d.splice(m, 1);
          for (var w = 0; w < a.length; ++w) a[w] === _ && a.splice(w, 1);
        }
      ...
      var a = function(s, i, c, t, p, l, o) {
        if (r.helper.isUndefined(o) && (o = ""), c = c.toLowerCase(), s.search(/\(<?\s*>? ?(['"].*['"])?\)$/m) > -1) t = "";
        else if (!t) if (c || (c = i.toLowerCase().replace(/ ?\n/g, " ")), t = "#" + c, !r.helper.isUndefined(d.gUrls[c])) t = d.gUrls[c], r.helper.isUndefined(d.gTitles[c]) || (o = d.gTitles[c]);
        else return s;
      ...
      return e = e.replace(/\[((?:\[[^\]]*]|[^\[\]])*)] ?(?:\n *)?\[(.*?)]()()()()/g, a), e = e.replace(/\[((?:\[[^\]]*]|[^\[\]])*)]()[ \t]*\([ \t]?<([^>]*)>(?:[ \t]*((["'])([^"]*?)\5))?[ \t]?\)/g, a), e = e.replace(/\[((?:\[[^\]]*]|[^\[\]])*)]()[ \t]*\([ \t]?<?([\S]+?(?:\([\S]*?\)[\S]*?)?)>?(?:[ \t]*((["'])([^"]*?)\5))?[ \t]?\)/g, a), e = e.replace(/\[([^\[\]]+)]()()()()()/g, a), u.ghMentions && (e = e.replace(/(^|\s)(\\)?(@([a-z\d]+(?:[a-z\d.-]+?[a-z\d]+)*))/gmi, function(s, i, c, t, p) {
        if (c === "\\") return i + t;
        if (!r.helper.isString(u.ghMentionsLink)) throw new Error("ghMentionsLink option must be a string");
    ...
    var Z = /([*~_]+|\b)(((https?|ftp|dict):\/\/|www\.)[^'">\s]+?\.[^'">\s]+?)()(\1)?(?=\s|$)(?!["<>])/gi, Q = /([*~_]+|\b)(((https?|ftp|dict):\/\/|www\.)[^'">\s]+\.[^'">\s]+?)([.!?,()\[\]])?(\1)?(?=\s|$)(?!["<>])/gi, X = /()<(((https?|ftp|dict):\/\/|www\.)[^'">\s]+)()>()/gi, J = /(^|\s)(?:mailto:)?([A-Za-z0-9!#$%&'*+-/=?^_`{|}~.]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)(?=$|\s)/gmi, Y = /<()(?:mailto:)?([-.\w]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)>/gi, R = function(e) {
      "use strict";
    ...
    r.subParser("autoLinks", function(e, u, d) {
      "use strict";
      return e = d.converter._dispatch("autoLinks.before", e, u, d), e = e.replace(X, R(u)), e = e.replace(Y, U(u, d)), e = d.converter._dispatch("autoLinks.after", e, u, d), e;
    }), r.subParser("simplifiedAutoLinks", function(e, u, d) {
      "use strict";
      return u.simplifiedAutoLink && (e = d.converter._dispatch("simplifiedAutoLinks.before", e, u, d), u.excludeTrailingPunctuationFromURLs ? e = e.replace(Q, R(u)) : e = e.replace(Z, R(u)), e = e.replace(J, U(u, d)), e = d.converter._dispatch("simplifiedAutoLinks.after", e, u, d)), e;
...
`;
      ...
      for (var a = 0; a < d.gHtmlSpans.length; ++a) {
        for (var s = d.gHtmlSpans[a], i = 0; /¨C(\d+)C/.test(s); ) {
          var c = RegExp.$1;
          if (s = s.replace("\xA8C" + c + "C", d.gHtmlSpans[c]), i === 50) {
...
`;
      ...
      function l(o, h, n, f, _, m, w, g) {
        ...
        var k = '<img src="' + f + '" alt="' + h + '"';
        return g && r.helper.isString(g) && (g = g.replace(/"/g, "&quot;").replace(r.helper.regexes.asteriskDashAndColon, r.helper.escapeCharactersCallback), k += ' title="' + g + '"'), _ && m && (_ = _ === "*" ? "auto" : _, m = m === "*" ? "auto" : m, k += ' width="' + _ + '"', k += ' height="' + m + '"'), k += " />", k;
      }
